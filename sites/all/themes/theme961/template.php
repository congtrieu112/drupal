...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme961
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme961_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme961_html_head_alter( &$head_elements ) {
 ...
 */
function theme961_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme961_preprocess( &$vars, $hook ) {
	$vars['theme961_path'] = base_path() . path_to_theme();
 ...
 */
function theme961_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme961_doctype();
	$vars['rdf'] = _theme961_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme961_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme961_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme961_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme961_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme961_blog_title' );
	}
}
 ...
 */
function theme961_preprocess_page( &$vars ) {
 ...
 */
function theme961_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme961' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme961_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme961_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme961_menu_tree( $vars ) {
 ...
 */
function theme961_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme961_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme961_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme961_breadcrumb_separator' );

	if ( theme_get_setting( 'theme961_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme961_tabs_float() {
	$float = (bool) theme_get_setting( 'theme961_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme961_tabs_node' );
...
function theme961_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme961_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme961_id_safe( $element['#title'] );
 ...
 */
function theme961_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme961_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme961_preprocess_comment( &$variables ) {
...
/* Region options */
function theme961_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme961_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'theme961_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'theme961_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'theme961_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'theme961_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'theme961_' . $reg . '_fullwidth' );
...
function theme961_region_suffix ( $reg ) {
	$fullwidth = theme_get_setting( 'theme961_' . $reg . '_fullwidth' );
	$block_bg_parallax = theme_get_setting( 'theme961_' . $reg . '_bg_parallax' );
	$block_bg_img = theme_get_setting( 'theme961_' . $reg . '_bg_img' );
...
function theme961_form_contact_site_form_alter( &$form, &$form_state, $form_id ) {
	...
	//$form['subject']['#element_validate'] = array( '_theme961_empty_field' );
...
/*function _theme961_empty_field ( $element, &$form_state ) {
